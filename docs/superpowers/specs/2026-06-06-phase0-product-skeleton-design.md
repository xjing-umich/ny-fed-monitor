...
> **让任何人都能在一页之内，看懂：聪明钱在买什么、它到底值不值、现在的大环境如何。**

把三件本来分散又难懂的事——**机构持仓 / 个股估值 / 宏观流动性**——用 AI 在一个页面讲成人话。这是 ValueSider、AlphaSpread、GuruFocus 各自都缺的那一块。

**"完全实现"时的画面**（用来校准是否在靠近这颗星）：访客搜"巴菲特 苹果"或被 AI 引用进来，落到一页即见：①谁在买(巴菲特及多少超级投资者持有 AAPL、加减仓、自申报浮盈) ②值不值(内在价值 vs 现价、高/低估、DCF+巴菲特+格林沃尔德多视角) ③大环境(当前流动性/利率对估值的含义) ④一段 AI 人话把三者串成"所以呢"，标清来源与日期。中英双语、移动端顺畅、干净不吓人、可一键分享。

**各阶段如何逼近这颗星：**
| 阶段 | 点亮哪一块 |
|------|------|
| Phase 0 骨架 | 造出"一页之内、人话、双语、移动端、可分享"的**容器**（实体页模板=这颗星的物理形态） |
| Phase 1 数据+日更价 | 点亮「谁在买」：更多超级投资者、多季历史、自申报浮盈 |
| Phase 2 分析+基本面 | 打通三视角的「联系」：跨经理人共识、个股被谁持有、基本面进场 |
| Phase 3 估值+SEO/GEO | 点亮「值不值」(逼近 AlphaSpread) + 让这颗星被搜索/AI 看见 |
...
**结论：继续用 serverless Vercel + Supabase，无需架构扩展；为重活补一条 GitHub Actions 批处理胳膊；大陆访问问题整体后置——先把公开网络(海外)版本做扎实。**

| 工作负载 | serverless 能否扛 | 处置 |
|------|------|------|
| 网站服务 / SSR/SSG/SEO/ISR | ✅ 强项 | Vercel 直接做，实体页静态化+ISR 对 SEO/GEO 最优 |
| AI 叙述 | ✅ | DeepSeek 服务端 + Supabase 缓存（现成模式） |
| 估值计算(P3) | ✅ | DCF/相对估值用 TS 确定性计算；叙述用 DeepSeek 预计算+缓存 |
| 13F 历史回填(P1) | ⚠️ 超 Vercel 函数 300s | → **GitHub Actions** 批处理写 Supabase |
| 日更股价(P1) | ⚠️ ~1500 ticker×Finnhub 60/分≈25 分 | → **GitHub Actions** 日更 |
| 基本面拉取(P2) | ⚠️ 周期性批量 | → **GitHub Actions** |
| 中国大陆访问 | ❌ Vercel/Supabase 大陆慢/不稳 | **后置**：先做海外公开网络；待 zh 真实流量再议(国内镜像+备案) |

- **GitHub Actions 批处理**本就在原 `phaseA` 架构计划内（免费、无 300s 限制），不是新增负担；Vercel 只管 serving + 轻量 cron。
- Supabase 免费档 500MB：50 经理人×~20 季×数百持仓 + 价格/基本面，估算初期可容纳，增长后升 Pro($25/月)。
