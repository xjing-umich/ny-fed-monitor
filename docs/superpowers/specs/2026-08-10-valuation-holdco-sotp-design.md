...
**五闸 fail-closed**（任一不过 → 退回件④的抑制，不给 SOTP）：
1. `kind==="operating"` 的分部至少 **3 个年份**可得；
2. **对账（两段，取代原「≤10% 单闸」）**：
   - **②-1 段内恒等式（决定性）**：各顶层业务分部税前之和 == 申报的分部合计行，**容差 1%**。这是真恒等式，拦「漏一个分部 / 把子分部当顶层 / 把合计当分部」——正是原 §1.1 那批事故形态，比 10% 单闸更早更严。
   - **②-2 跨管线量级对账**：分部合计与合并报表口径（`pretax_income − 投资重估损益`，按 `period_end` 配对；对不上直接 `reconciliation_unavailable` fail-closed）偏差 **≤35%**。这是**量级闸**不是精度闸：拦整体解析错位、错把合并税前当分部、错把单一分部当合计（这类事故形态偏差 ≥86%，与真实最坏 19.2% 之间隔一整个数量级）。**为什么不是 10%**：BRK FY2025 分部合计对合并口径实测 **+19.2%**（成因是分部口径不含权益法减值等公司层减项，真实且每年发生），10% 单闸会把伯克希尔**本身**误抑制。
3. **第一栏归属闸**：第一栏所取的现金 + 其余分列现金 + 受限现金 ≈ 合并现金总额（容差 2%）——拦「漏取某一列」；
4. **第一栏闭合闸**：各 `ProductOrService` 顶层 `Assets` 之和 ≈ `Assets` 合并数（容差 2%）——拦「存在未被发现的第三个资产池」；
5. **第一栏上界闸**：第一栏 `total ≤`「保险与其他」列的 `Assets` ×（1+容差）——拦「组成项回退到无维度合并值 → 混入已算进第二栏的经营资产 → 第一栏系统性偏高」。归属闸只验现金、闭合闸只验 Assets 合计，都拦不住这类高估，故需第三条恒等式。

闸 3/4/5 是 §1.1 那个 321B 漏项的直接产物：**光靠「字段非空」判齐备是不够的，必须用会计恒等式把漏项与重复计入都逼出来。**

> **收紧遗留（合并后另案）**：②-2 的 35% 是量级闸而非精度闸。若把合并口径基数换成 instance 里的 `…MinorityInterestAndIncomeLossFromEquityMethodInvestments`（BRK 偏差降到 2.4%）并存进 `company_segment_years`，可把 ②-2 收回 10% 量级，且不必碰 `company_fundamentals_periods`（避开加列断更风险）。
...
1. 纯 fixture check：**五闸**各自 fail-closed；保险**投资分部**被排除；**日元/欧元/英镑单位事实被拒绝**；**第一栏缺 `USTreasuryBills` 时闸 3/4/5 必须拦下**（用本次的真实漏项当回归用例）；**段内恒等式（②-1）在漏一个分部时必须拦下**。
2. 真数据探针（只读）：
   - BRK.B 第一栏 = **704.73B ± 1%**，且逐项等于 §1.1 表；
   - 分部解析出 **≥3 年**、段内恒等式（②-1）过、跨管线量级对账（②-2）三年实测 **+19.2% / −6.3% / −3.7%** 均在 35% 内、非保险经营税后三年为 **21.74 / 22.38 / 23.37B ± 1%**；
