...
### §9.2 HRB 增量法独立对账 —— PASS(裁定:15% 季节性偏差门槛已废弃,改逐字段独立对账)

首次验收时用"|TTM_NI−FY_NI|/FY_NI < 15%"作为季节性抵消的启发式断言,被真数据证伪(实际偏差 22.1%)。
经手工独立复核(直查 `company_fundamentals_periods`,不复用引擎路径)确认增量法计算本身逐字段正确,
证伪的是"偏差应为小个位数%"这条设计期猜测,不是代码。裁定改用 **GOOGL 同款独立对账断言**(revenue→net_income),
只验算术正确性,不再对偏差幅度设门槛:

```
独立复算: FY(2025-06-30)=605,773,000
  + Σ新季度[2025-09-30=−165,819,000, 2025-12-31=−242,166,000, 2026-03-31=847,901,000]
  − Σ去年同期[2024-09-30=−172,576,000, 2024-12-31=−243,420,000, 2025-03-31=722,330,000]
  = 739,355,000
