...
// 标签复用 ValuationBadge 同款位置词(击球区/低于价值带/带内/高于价值)。纯位置陈述, 无荐买卖。
const COPY = {
  zh: { strike: "击球区", below: "低于价值带", within: "带内", above: "高于价值" },
  en: { strike: "Strike zone", below: "Below value", within: "Within band", above: "Above value" },
} as const;

/**
 * 估值 verdict → masthead 徽章 {label,tone}。null → 无徽章。
 * 便宜档(击球区/低于价值带)可信才 positive; 红旗(reliable=false)降 neutral(不标已确认便宜,
